...
def generate_exon_to_end(file_from, prefix=''):
    with open(file_from) as in_file, open(prefix + 'exons_end_start_0.txt', 'w') as ex0,\
                    open(prefix + 'exons_end_start_1.txt', 'w') as ex1,\
                    open(prefix + 'exons_end_start_2.txt', 'w') as ex2:
        for line in in_file:
            print(line)
            previous = 2
