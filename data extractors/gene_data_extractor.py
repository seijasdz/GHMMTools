...
def slicer(string, divisions, complement, before=50, after=50):
    def reverse(s):
        return s[::-1]
    ...
    for i, div in enumerate(divisions):
        ...
        if i < len(divisions) - 1:
            if div[1] < divisions[i + 1][0] and len(string[div[1]: divisions[i + 1][0]]) > 2:
                ...
                data['introns_acceptor'].append(string[div[1]: divisions[i + 1][0] + 38])
...
def extractor(folder_path):
    ...
    for key, gene in genes.items():
        if not gene['complement']:
            ...
            introns_acceptor.append(' '.join(gene['introns_acceptor']) + '\n')
    ...
    with open('new_cuts.txt', 'w') as cuts, open('new_cutsa.txt', 'w') as acuts, open('new_cutsb.txt', 'w') as bcuts, open('new_cuts_intron.txt', 'w') as intron_file, open('new_intron_acceptor.txt', 'w') as i_acceptor_file:
        ...
        for intron_acceptor in introns_acceptor:
            single = intron_acceptor.split(' ')
            for c, i in enumerate(single):
                if c > 2:
                    if '\n' not in i:
                        i_acceptor_file.write(i + '\n')
                    else:
                        i_acceptor_file.write(i)

    with open('new_ccuts.txt', 'w') as cuts, open('new_ccutsa.txt', 'w') as acuts, open('new_ccutsb.txt', 'w') as bcuts:
